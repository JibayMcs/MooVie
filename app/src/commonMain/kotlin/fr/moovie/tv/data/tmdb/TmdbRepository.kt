...
class TmdbRepository(
    ...
    private val api = TmdbApi(clientTmdb)
    ...
    suspend fun validateKey(apiKey: String): KeyCheck = runCatching {
        ...
        onFailure = { error ->
            // `ResponseException` est ce que lève Ktor sous `expectSuccess`,
            // et son sous-type `ClientRequestException` couvre les 4xx. On lit
            // le statut plutôt que le type : seul 401 met en cause la clé, un
            // 404 ou un 429 sont des pannes comme les autres.
            if (error is ResponseException && error.response.status.value == 401) {
                KeyCheck.REJECTED
            } else {
